...
const formsRetrieveObject = {
  ...
  getNewItemFormInfo() {
    ...
    newItemFormData.addEventListener("click", (e) => {
      ...
      const newItemFormChecklist = document.getElementById(
        "itemFormChecklistUL",
      );
      ...
      for (let i = 0; i < checklistTally; i++) {
        ...
        children[i].firstElementChild.nextElementSibling.id =
          `tempCheckbox${i}`;
      }
  ...
  getEditFormInfo(categoryindex, itemindex) {
    ...
    submitEditFormData.addEventListener("click", (e) => {
      ...
      if (category != array[categoryindex]["category"]) {
        console.log(
          "Changing the category. Data: the old category index = " +
            categoryindex,
        );
        ...
        let newCategoryIndex = array.findIndex(
        ...
        console.log(
          "new project item priority " +
            projectsObject.getItem(newCategoryIndex, newItemIndex).priority,
        );
